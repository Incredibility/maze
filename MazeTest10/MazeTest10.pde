Maze[] = {py1, py2, py3, py4};
int py1_clm = 16, py1_row = 16, py2_clm = 16, py2_row = 16, py3_clm = 16, py3_row = 16, py4_clm = 16, py4_row = 16, ply_num = 4, maz_num, wnr;

void setup () {
  size(1280, 720);
  rectMode(CORNERS);
  focused = true;
  int dim_max = min(width, height) == height ? floor(0.525*height) : floor(0.525*width);
  py1_clm = constrain(py1_clm, 2, dim_max);
  py1_row = constrain(py1_row, 2, dim_max);
  py2_clm = constrain(py2_clm, 2, dim_max);
  py2_row = constrain(py2_row, 2, dim_max);
  py1 = new Maze(py1_clm, py1_row);
  if (ply_num > 1) py2 = new Maze(py2_clm, py2_row);
  if (ply_num > 2) py3 = new Maze(py3_clm, py3_row);
  if (ply_num = 4) py4 = new Maze(py4_clm, py4_row);
  background(250);
  stroke(130);
  strokeWeight(py1.sw);
...
class Maze {
  ...
  Cell[][] cll;
  Maze(int clm_, int row_) {
    clm = clm_;
    row = row_;
    boolean hil = min(width, height) == height ? true : false;
    float lgh = 0.9*height, sml = hil ? 0.05*height : 0.05*width;
    ;
    if (ply_num == 1) {
      w = hil ? lgh/row : 0.9*width/clm;
      xmg = sml;
      ymg = sml;
    } else if (ply_num == 2) {
      w = hil ? lgh : 0.4*width;
      xmg = maz_num == 0 ? sml : hil ? 0.5*height + sml : 0.5*width + sml;
      ymg = sml;
    } else if (ply_num == 3) {
      w =
    }
