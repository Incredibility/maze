int[][] dim = {{16, 16}, {16, 16}}; //p1c p1r   p2c p2r
Maze[] py = new Maze[2];
Maze wnr;
...
void setup() {
  ...
  rectMode(CORNERS);
  focused = true;
  for (int[] dim1 : dim) for (int dim : dim1) dim = constrain(dim, 2, 378);
  boolean hil = width > height ? true : false;
  boolean aln_rht = false;
  float h = hil ? 0.05*height : 0.05*width;
  float[] pos = {hil ? 0.55*width : 0.55*height, h};
  for (int i = 0; i < py.length; i++) {
    if (i == 0) {
      float[] pos_ = {hil ? 0.45*width : 0.45*height, h};
      pos = pos_;
      aln_rht = true;
    }
    py[i] = new Maze(i, dim[i], pos, aln_rht);
  }
}
...
class Maze {
  ...
  Cell[][] cll;
  Maze(int py_, int[] dim_, float[]pos_, boolean aln_rht_) {
    dim = dim_;
    w = height < width ? 0.9*height/dim[1] : 0.4*width/dim[0];
    hw = max(1, w/2);
    pos[0] = aln_rht_ ? pos_[0] - dim[0]*w : pos_[0];
    pos[1] = pos_[1];
    sw = dim[0] > dim[1] ? max(1, 128/dim[0]) : max(1, 128/dim[1]);
    ssw = sw/4;
    cll = new Cell[dim[0]][dim[1]];
    for (int i = 0; i < dim[0]; i++) for (int j = 0; j < dim[1]; j++) {
      int[] cr = {i, j};
      cll[i][j] = new Cell(py_, cr);
    }
    ...
    while (!stk.isEmpty()) {
      ...
      if (gen_crt.cr[1] != 0 && !cll[gen_crt.cr[0]][gen_crt.cr[1] - 1].vst) nbr.add(cll[gen_crt.cr[0]][gen_crt.cr[1] - 1]);
      if (gen_crt.cr[0] != dim[0] - 1 && !cll[gen_crt.cr[0] + 1][gen_crt.cr[1]].vst) nbr.add(cll[gen_crt.cr[0] + 1][gen_crt.cr[1]]);
      if (gen_crt.cr[1] != dim[1] - 1 && !cll[gen_crt.cr[0]][gen_crt.cr[1] + 1].vst) nbr.add(cll[gen_crt.cr[0]][gen_crt.cr[1] + 1]);
      if (gen_crt.cr[0] != 0 && !cll[gen_crt.cr[0] - 1][gen_crt.cr[1]].vst) nbr.add(cll[gen_crt.cr[0] - 1][gen_crt.cr[1]]);
      if (!nbr.isEmpty()) {
        stk.add(gen_crt);
        Cell nxt = nbr.get(floor(random(nbr.size())));
        if (gen_crt.cr[1] - nxt.cr[1] == 1) gen_crt.wll[0] = nxt.wll[2] = false;
        else if (gen_crt.cr[1] - nxt.cr[1] == -1) gen_crt.wll[2] = nxt.wll[0] = false;
        if (gen_crt.cr[0] - nxt.cr[0] == 1) gen_crt.wll[3] = nxt.wll[1] = false;
        else if (gen_crt.cr[0] - nxt.cr[0] == -1) gen_crt.wll[1] = nxt.wll[3] = false;
        gen_crt = nxt;
      } else gen_crt = stk.remove(stk.size() - 1);
    }
  }
}
...
class Cell {
  ...
  Cell(int pyy, int[] cr_) {
    Maze py3 = py[pyy];
    cr = cr_;
    x1 = cr[0]*py3.w + py3.pos[0];
    y1 = cr[1]*py3.w + py3.pos[1];
    x2 = x1 + py3.w;
    y2 = y1 + py3.w;
  }
}
